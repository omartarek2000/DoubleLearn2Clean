        ...
        if (type(Lpath) != list):
            ...
            if (len(Lpath) == 1):
                ...
                if (target_name in df.columns):
                    ...
                    # for train_index, test_index in skf.split(df, df[target_name]):
                    #     df_train, df_test = df[train_index], df[test_index]
                    #     y_train, y_test = df[target_name][train_index], df[target_name][test_index]
                    df_train, df_test, y_train, y_test = train_test_split(
                        df, df[target_name], test_size=0.33)

                    # df_train, df_test = ros.fit_resample(df_train, df_test)
                    # y_train, y_test = ros.fit_resample(y_train, y_test)
                ...
                for path in Lpath:
                    ...
                    if (target_name in df.columns):
                        ...
                        # df_train[path], df_test[path] = ros.fit_resample(df_train[path], df_test[path])
                        # y_train[path], y_test[path] = ros.fit_resample(y_train[path], y_test[path])
            ...
            #comment the next 2 lines if running Double QLearning without Random Overssampling
