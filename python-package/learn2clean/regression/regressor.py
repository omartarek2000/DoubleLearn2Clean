   ...
   """
    ...
    def OLS_regression(self, dataset, target):  # quality metrics : accuracy
        ...
        if (len(X_train.columns) <= 1) or (len(X_train) < k):
            ...
            display(resReg.params)
            display(X1Test)

            display(resReg.params.shape)
            display(X1Test.shape)

            while resReg.params.shape[0] > X1Test.shape[1]: 
                resReg.params = resReg.params.iloc[:-1]
            while resReg.params.shape[0] < X1Test.shape[1]: 
                X1Test = X1Test.iloc[:, :-1]

            display(resReg.params.shape)
            display(X1Test.shape)
            # display(resReg.params.iloc[:,:-1])
            # display(X1Test)
